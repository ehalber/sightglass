(function ($) {
  ...
  var methods = {
    init: function(options) {
      ...
      var imagesWithoutSize = $('img:not([height]), img:not([width])', this);
      if (!imagesWithoutSize.length) {
        _this.cycle(_this.data());
      } else {
        imagesWithoutSize.one('load error', function() {
    ...
    pagerAnchorBuilder: function(i) {
      $(this).data('updateActivePagerLink', methods.updateActivePagerLink);
      return $('<li>').html(i + 1);
    },
